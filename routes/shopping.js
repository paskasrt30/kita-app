...
router.get('/active', (req, res) => {
    ...
    if (!list) {
        ...
        list = db.prepare('SELECT * FROM shopping_lists WHERE id = ?').get(result.lastInsertRowid);
    }
...
// ==================== TAMBAH ITEM BELANJA ====================
router.post('/:list_uuid/items', (req, res) => {
    try {
        ...
        const list = db.prepare('SELECT * FROM shopping_lists WHERE uuid = ? AND couple_id = ?')
...
router.post('/:uuid/complete', (req, res) => {
    ...
    db.prepare("UPDATE shopping_lists SET status = 'selesai' WHERE uuid = ?").run(req.params.uuid);
