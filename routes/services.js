...
router.get('/', (req, res) => {
    const items = db.prepare('SELECT * FROM service_schedules WHERE couple_id = ? ORDER BY tanggal_servis_berikutnya ASC').all(req.user.couple_id);
...
router.post('/', (req, res) => {
    try {
        ...
        const newItem = db.prepare('SELECT * FROM service_schedules WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:uuid/done', (req, res) => {
    const item = db.prepare('SELECT * FROM service_schedules WHERE uuid = ? AND couple_id = ?')
    ...
    const updated = db.prepare('SELECT * FROM service_schedules WHERE uuid = ?').get(req.params.uuid);
...
router.delete('/:uuid', (req, res) => {
    ...
    db.prepare('DELETE FROM service_schedules WHERE uuid = ?').run(req.params.uuid);
