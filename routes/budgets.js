...
router.post('/', (req, res) => {
    try {
        ...
        if (existing) {
            db.prepare('UPDATE budgets SET target_nominal = ? WHERE id = ?').run(target_nominal, existing.id);
        } else {
...
router.delete('/:id', (req, res) => {
    const budget = db.prepare('SELECT * FROM budgets WHERE id = ? AND couple_id = ?')
