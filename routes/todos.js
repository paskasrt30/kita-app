...
router.put('/:uuid', (req, res) => {
    const todo = db.prepare('SELECT * FROM todos WHERE uuid = ? AND couple_id = ?')
    ...
    const updated = db.prepare('SELECT * FROM todos WHERE uuid = ?').get(req.params.uuid);
...
router.delete('/:uuid', (req, res) => {
    ...
    db.prepare('DELETE FROM todo_comments WHERE todo_id = ?').run(todo.id);
    db.prepare('DELETE FROM todos WHERE uuid = ?').run(req.params.uuid);
...
// ==================== KOMENTAR: TAMBAH ====================
router.post('/:uuid/comments', (req, res) => {
    const todo = db.prepare('SELECT * FROM todos WHERE uuid = ? AND couple_id = ?')
        .get(req.params.uuid, req.user.couple_id);
    ...
    db.prepare('INSERT INTO todo_comments (todo_id, user_id, komentar) VALUES (?, ?, ?)')
