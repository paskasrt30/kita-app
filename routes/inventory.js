...
router.get('/', (req, res) => {
    const items = db.prepare('SELECT * FROM inventory_items WHERE couple_id = ? ORDER BY created_at DESC').all(req.user.couple_id);
...
router.put('/:uuid', (req, res) => {
    const item = db.prepare('SELECT * FROM inventory_items WHERE uuid = ? AND couple_id = ?')
    ...
    const updated = db.prepare('SELECT * FROM inventory_items WHERE uuid = ?').get(req.params.uuid);
...
router.delete('/:uuid', (req, res) => {
    ...
    db.prepare('DELETE FROM inventory_items WHERE uuid = ?').run(req.params.uuid);
